...
def object_detection_ocr(image, confidence):
    ...
    model = models.get('yolo_nas_s', 
                        num_classes=1, 
                        checkpoint_path='./Checkpoints/ckpt_best.pth', fuse_model=False).to(device)  # Load the object detection model
    ...
    # Ensure result is not treated as an iterable if it's not structured that way
